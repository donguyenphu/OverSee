  ...
  return (
    <form onSubmit={handleSubmit} className='space-y-6 p-6 rounded-lg border bg-white shadow'>
      ...
      <div className='grid md:grid-cols-2 gap-4'>
        ...
        <div><label className='text-sm font-medium mb-1 block'>Facebook *</label><Input name='facebook' value={data.facebook} onChange={handleChange} placeholder='Facebook' required maxLength={100} /></div>
        ...
        <div><label className='text-sm font-medium mb-1 block'>Lớp *</label><Input type='number' min='1' max='12' name='grade' value={data.grade} onChange={handleChange} placeholder='Lớp đang theo học' required /></div>
        ...
        <div><label className='text-sm font-medium mb-1 block'>Điểm hiện tại *</label><Input type='number' step='0.1' min='0' max='10' name='currentResult' value={data.currentResult} onChange={handleChange} placeholder='Điểm/kết quả môn học đó trên lớp hiện tại' required /></div>
        <div><label className='text-sm font-medium mb-1 block'>Mục tiêu *</label><Input type='number' step='0.1' min='0' max='10' name='target' value={data.target} onChange={handleChange} placeholder='Mục tiêu đối với môn học đó' required /></div>
      </div>
      <div><label className='text-sm font-medium mb-1 block'>Nguyện vọng thêm *</label><Textarea name='wishes' value={data.wishes} onChange={handleChange} placeholder='Chia sẻ nguyện vọng thêm từ học sinh/phụ huynh (Không có thì ghi "Không có")' required maxLength={500} /></div>

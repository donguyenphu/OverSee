  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="w-full max-w-screen-2xl mx-auto px-6 py-10 flex-grow">
        {step === 'listening' && <Listening userEmail={userEmail} onComplete={results => { setListeningAnswers(results.answers); complete('Hoàn thành Listening! Bắt đầu Reading', 'reading'); }} audioUrl={test.audioUrl} />}
        {step === 'reading' && <Reading userEmail={userEmail} onComplete={results => { setReadingAnswers(results.answers); complete('Hoàn thành Reading! Bắt đầu Writing', 'writing'); }} />}
        {step === 'writing' && <Writing userEmail={userEmail} onComplete={(results: WritingResults) => { setWritingTask1(results.task1); setWritingTask2(results.task2); complete('Hoàn thành Writing! Xem kết quả', 'results'); }} />}
        {step === 'results' && <Results testId={test.id} userEmail={userEmail} listeningAnswers={listeningAnswers} readingAnswers={readingAnswers} writingTask1={writingTask1} writingTask2={writingTask2} onBackToHome={() => navigate('/')} />}
      </main>
      <Footer />
    </div>
  );
